...
exports.login = asyncHandler(async (req, res, next) => {
    ...
    if (!email || !password) {
        return next(
            new ErrorResponse('Please provide email and passowrd', 400),
        );
    }
...
//@desc         Log out/ Clear token cookie
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new ErrorResponse('The user not found with that email', 404),
        );
    }
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
    ...
    if (user.resetPasswordExpire < Date.now()) {
        await user.save();
        return next(new ErrorResponse(`Reset password time expired`, 400));
    }
